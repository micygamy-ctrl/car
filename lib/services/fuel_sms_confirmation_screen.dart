...
class _FuelSmsConfirmationScreenState
  ...
  Future<void> _loadCurrency() async {
    ...
    setState(() => _currency = prefs.getString('currency') ?? 'EGP');
  }
  ...
  Future<void> _saveFuelLog() async {
    ...
    try {
      ...
      final pricePerUnit =
          double.tryParse(_pricePerLiterController.text) ?? 0;
      ...
      final log = FuelLogModel(
        ...
        notes: 'تم اكتشافه من رسالة ${widget.smsResult.bankName}',
      );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('تم تسجيل الوقود ✅', style: GoogleFonts.cairo()),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _infoRow('البنك', widget.smsResult.bankName),
                    _infoRow('المحطة', widget.smsResult.merchantName),
                    _infoRow(
                      'المبلغ',
                      '${widget.smsResult.amountEGP.toStringAsFixed(2)} $_currency',
                    ),
                ...
                child: widget.cars.isEmpty
                    ...
                    : Column(
                        children: widget.cars.map((car) {
                          final selected = _selectedCar?.carId == car.carId;
